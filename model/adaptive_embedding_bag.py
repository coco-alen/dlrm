...
class AdaptiveEmbeddingBag(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, cutoffs, div_value=4.):
        super(AdaptiveEmbeddingBag, self).__init__()
        ...
        for i in range(self.n_clusters):
            ...
            projection = nn.Sequential(
                nn.EmbeddingBag(vsz, hsz),
                nn.Linear(hsz, self.embedding_dim, bias=False)
            )

            self.projections.append(projection)

    def forward(self, emb_input: torch.Tensor, offsets=None, per_sample_weights=None):
        ...
        for i in range(self.n_clusters):
            ...
            # cluster_output = self.projections[i](input_subset, offsets = offsets, per_sample_weights = per_sample_weights)
